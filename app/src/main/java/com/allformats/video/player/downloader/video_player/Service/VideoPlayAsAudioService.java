...
import com.allformats.video.player.downloader.video_player.Util.Vid_player_VideoPlayerUtils;
...
public class VideoPlayAsAudioService extends Service {
    ...
    private final BroadcastReceiver mIntentReceiver = new BroadcastReceiver() { // from class: com.allformats.video.player.downloader.video_player.Service.VideoPlayAsAudioService.1
        @Override 
    ...
    ArrayList<Vid_player_MediaData> videoList = new ArrayList<>();
    ...
    public int onStartCommand(Intent intent, int i, int i2) {
        if (intent != null) {
            if (intent.getAction() == null || !intent.getAction().equals(NOTIFICATION_CLICK_ACTION)) {
                if (!this.isUpdate) {
                    videoPosition = intent.getIntExtra(Vid_player_Constant.EXTRA_VIDEO_POSITION, 0);
                    videoPlay();
                    this.isUpdate = true;
                }
    ...
    private Notification buildNotification() {
        final RemoteViews remoteViews = new RemoteViews(getPackageName(), R.layout.vid_player_layout_notification_collapse);
        final RemoteViews remoteViews2 = new RemoteViews(getPackageName(), R.layout.vid_player_layout_notification);
        String name = ((Vid_player_MediaData) this.videoList.get(videoPosition)).getName();
        boolean isPlaying = this.mediaPlayer.isPlaying();
        StringBuilder stringBuilder = new StringBuilder();
        stringBuilder.append(Vid_player_VideoPlayerUtils.makeShortTimeString(this, (long) (this.mediaPlayer.getCurrentPosition() / 1000)));
        stringBuilder.append(" / ");
        stringBuilder.append(Vid_player_VideoPlayerUtils.makeShortTimeString(this, (long) (((Vid_player_MediaData) this.videoList.get(videoPosition)).getVideoDuration() / 1000)));
        String stringBuilder2 = stringBuilder.toString();
        int i = isPlaying ? R.drawable.ic_pause : R.drawable.ic_play;
        PendingIntent activity = PendingIntent.getActivity(this, 0, new Intent(Vid_player_VideoPlayerActivity.getIntent(this, this.videoList, videoPosition, true)), PendingIntent.FLAG_UPDATE_CURRENT);
        File file = new File(((Vid_player_MediaData) this.videoList.get(videoPosition)).getPath());
        RequestBuilder load = Glide.with((Context) this).load(((Vid_player_MediaData) this.videoList.get(videoPosition)).getPath());
        RequestOptions requestOptions = new RequestOptions();
        StringBuilder stringBuilder3 = new StringBuilder();
        stringBuilder3.append(file.getAbsolutePath());
        stringBuilder3.append(file.lastModified());
        load.apply((RequestOptions) ((RequestOptions) ((RequestOptions) requestOptions.signature(new ObjectKey(stringBuilder3.toString()))).priority(Priority.LOW)).diskCacheStrategy(DiskCacheStrategy.RESOURCE)).transition(DrawableTransitionOptions.withCrossFade()).into(new SimpleTarget<Drawable>() {
        ...
        remoteViews.setTextViewText(R.id.artistName, stringBuilder2);
        remoteViews.setImageViewResource(R.id.play_pause, i);
        remoteViews2.setTextViewText(R.id.songName, name);
        remoteViews2.setTextViewText(R.id.artistName, stringBuilder2);
        remoteViews2.setImageViewResource(R.id.play_pause, i);
        this.notificationBuilder.setSmallIcon(2131231036).setContentIntent(activity).setCustomContentView(remoteViews).setCustomBigContentView(remoteViews2);
        if (isPlaying) {
            name = PAUSE_ACTION;
            remoteViews.setOnClickPendingIntent(R.id.play_pause, retrievePlaybackAction(name));
            remoteViews2.setOnClickPendingIntent(R.id.play_pause, retrievePlaybackAction(name));
        } else {
            name = PLAY_ACTION;
            remoteViews.setOnClickPendingIntent(R.id.play_pause, retrievePlaybackAction(name));
            remoteViews2.setOnClickPendingIntent(R.id.play_pause, retrievePlaybackAction(name));
        ...
        name = NEXT_ACTION;
        remoteViews.setOnClickPendingIntent(R.id.next, retrievePlaybackAction(name));
        String str = STOP_ACTION;
        remoteViews.setOnClickPendingIntent(R.id.close, retrievePlaybackAction(str));
        remoteViews2.setOnClickPendingIntent(R.id.next, retrievePlaybackAction(name));
        remoteViews2.setOnClickPendingIntent(R.id.previous, retrievePlaybackAction(PREVIOUS_ACTION));
        remoteViews2.setOnClickPendingIntent(R.id.close, retrievePlaybackAction(str));
    ...
    private PendingIntent retrievePlaybackAction(String str) {
        ...
        intent.putExtra(Vid_player_Constant.EXTRA_VIDEO_POSITION, videoPosition);
